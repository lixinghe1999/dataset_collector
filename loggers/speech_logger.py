...
def init_layout_speech():
    ...
    '''
    font_size = 14
    sensor_type = ['KN BPs USB', 'RØDE AI-Micro']
    ...
    speaker_type = ['AirPods Pro #2 Stereo', 'OpenMove by Shokz Stereo', 'none']
        ...
        if event == 'confirm':
            ...
            if not confirm:
                ...
                if event == 'start_record' and not isRecording:
                    ...
                    probe_signal = window['proble_signal'].get()[0]; speaker_type = window['speaker'].get()[0]
                    if probe_signal == 'none' or speaker_type == 'none':
                        play_command = 'echo "No probe signal selected, skipping playback."'
                    else:
                        probe_file = f"loggers/{probe_signal}.wav"
                        play_command = 'python utils/Audio/play.py --play_file {} --device {} --duration {}'.format(probe_file, play_indexs[0], duration+1)
                    _ = subprocess.Popen(play_command, shell=True)
